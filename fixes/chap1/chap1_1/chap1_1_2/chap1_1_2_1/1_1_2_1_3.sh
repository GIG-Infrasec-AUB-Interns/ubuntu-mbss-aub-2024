...
# 1.1.2.1.3 [REMEDIATION] Ensure nosuid option set on /tmp partition

{
    echo "[REMEDIATION] Ensuring nosuid option set on /tmp partition (1.1.2.1.3)..."
    ...
    if [[ -z "$findmnt_output" ]]; then
        ...
        if grep -q ' /tmp ' /etc/fstab; then
            echo "Updating /etc/fstab to include nosuid option for /tmp."
            sed -i '/ \/tmp / s/\(.*\)defaults\(.*\)/\1defaults,nosuid\2/' /etc/fstab
        else
        ...
        echo "nosuid option has been set on /tmp and remounted successfully."
