...
# 1.1.2.2.2 [REMEDIATION] Ensure nodev option set on /dev/shm partition

{
    echo "[REMEDIATION] Ensuring nodev option set on /dev/shm partition (1.1.2.2.2)..."
    ...
    if [[ -z "$findmnt_output" ]]; then
        ...
        if grep -q ' /dev/shm ' /etc/fstab; then
            echo "Updating /etc/fstab to include nodev option for /dev/shm."
            sed -i '/ \/dev/shm / s/\(.*\)defaults\(.*\)/\1defaults,nodev\2/' /etc/fstab
        else
            echo "/dev/shm entry not found in /etc/fstab. Adding entry with nodev option."
