...
# 1.1.1.7 Ensures udf kernel module is not available

{
    echo "Ensuring udf kernel module is not available (1.1.1.7)..."
    ...
    if [ -z "$l_output2" ]; then
        ...
        # Remediation
        read -p "Run remediation script for Test 1.1.1.7 (disables udf module, required by Microsoft Azure systems)? (Y/N): " ANSWER
        case "$ANSWER" in
            [Yy]*)
                echo "Commencing remediation for Test 1.1.1.7..."
                
                FIXES_SCRIPT="$(realpath fixes/chap1/chap1_1/chap1_1_1/1_1_1_7.sh)"
