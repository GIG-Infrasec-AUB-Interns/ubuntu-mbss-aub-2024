...
# 1.1.1.8 Ensures usb-storage kernel module is not available

{
    echo "Ensuring usb-storage kernel module is not available (1.1.1.8)..."
    ...
    if [ -z "$l_output2" ]; then
        ...
        # Remediation
        read -p "Run remediation script for Test 1.1.1.8 (disables usb-storage module, required by for USB storage device usage)? (Y/N): " ANSWER
        case "$ANSWER" in
            [Yy]*)
                echo "Commencing remediation for Test 1.1.1.8..."
                
                FIXES_SCRIPT="$(realpath fixes/chap1/chap1_1/chap1_1_1/1_1_1_8.sh)"
