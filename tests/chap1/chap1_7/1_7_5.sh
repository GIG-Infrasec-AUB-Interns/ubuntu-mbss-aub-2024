...
{
    echo "Ensuring GDM screen locks cannot be overridden (1.7.5)..."
    ...
    if [ -z "$l_output2" ]; then
        ...
        read -p "Run remediation script for Test 1.7.5? (Y/N): " ANSWER
            case $ANSWER in
                [Yy])
                    echo "Commencing remediation for Test 1.7.5..."

                    FIXES_SCRIPT="$(realpath fixes/chap1/chap1_7/1_7_5.sh)"
