#!/usr/bin/env bash
{
    echo "Ensuring GDM disabling automatic mounting of removable media is not overridden (1.7.7)..."
    ...
    if [ -z "$l_output2" ]; then
        ...
        read -p "Run remediation script for Test 1.7.7? (Y/N): " ANSWER
        case $ANSWER in
            [Yy])
                echo "Commencing remediation for Test 1.7.7..."

                FIXES_SCRIPT="$(realpath fixes/chap1/chap1_7/1_7_7.sh)"
