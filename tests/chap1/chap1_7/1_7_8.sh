#!/usr/bin/env bash
{
    echo "Ensuring GDM autorun-never is enabled (1.7.8)..."
    ...
    if [ -z "$l_output2" ]; then
        ...
        read -p "Run remediation script for Test 1.7.8? (Y/N): " ANSWER
        case $ANSWER in
            [Yy])
                echo "Commencing remediation for Test 1.7.8..."

                FIXES_SCRIPT="$(realpath fixes/chap1/chap1_7/1_7_8.sh)"
