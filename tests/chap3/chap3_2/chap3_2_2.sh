#!/usr/bin/env bash
{
    ...
    if [ -z "$l_output2" ]; then
        ...
        #Remediation 
        read -p "Run remediation script for Test 3.2.2? (Y/N): " ANSWER
        case "$ANSWER" in
            [Yy]*)
                echo "Commencing remediation for Test 3.2.2..."
                # Resolve absolute path to fixes/chap3/chap3_2/chap3_2_2.sh
                FIXES_SCRIPT="$(realpath fixes/chap3/chap3_2/chap3_2_2.sh)"
