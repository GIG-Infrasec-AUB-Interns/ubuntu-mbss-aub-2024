...
{ 
  ...
  if [ -z "$l_output2" ]; then 
    ...
    echo "NOTE: Before proceeding with remediation, always consult relevant vendor documentation to avoid breaking any application dependencies on a given file"
    runFix "7.1.11" fixes/chap7/chap7_1/7_1_11.sh # Remediation
  fi 
}