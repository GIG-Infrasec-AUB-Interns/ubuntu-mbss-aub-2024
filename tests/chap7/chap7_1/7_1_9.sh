...
{
  echo "Ensuring permissions on /etc/shells are configured (7.1.9)..."
  audit_output=$(stat -Lc 'Access: (%#a) Uid: (%u/%U) Gid: (%g/%G)' /etc/shells)
  ...
  if ([ $permissions -le 0644 ] && [ "$uid" == "0/root" ] && [ "$gid" == "0/root" ]); then
    ...
    runFix "7.1.9" fixes/chap7/chap7_1/7_1_9.sh # Remediation
  fi
}
