...
{ 
  echo "Ensuring local interactive user dot files access is configured (7.2.10)..."
  ...
  if [ -z "$l_output2" ]; then # If l_output2 is empty, we pass 
    ...
    echo "Before remediation, it is recommended that a monitoring policy be established to report user dot file permissions and determine the action to be taken in accordance with site policy."
    runFix "7.2.10" fixes/chap7/chap7_2/7_2_10.sh # Remediation
  fi
}